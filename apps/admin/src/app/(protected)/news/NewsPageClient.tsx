...
import { deleteNews, fetchNews, updateNews } from '@/actions/news'
import ConfirmDialog from '@/app/components/ConfirmDialog'
import LoadingSpinner from '@/app/components/LoadingSpinner'
import Toast, { ToastMessage } from '@/app/components/Toast'
import { NEWS_CATEGORY_OPTIONS, NewsCategory, NewsItem } from './types'
...
const getAuthorName = (item: NewsItem) => item.author.fullName || item.author.nickName
...
export default function NewsPageClient() {
  ...
  const loadNews = useCallback(async (showRefresh = false) => {
    try {
      ...
      const message = error instanceof Error ? error.message : 'Помилка завантаження новин'
  ...
  const handleTogglePublish = useCallback(
    async (item: NewsItem) => {
      try {
        ...
        addToast(item.published ? 'Новину знято з публікації' : 'Новину опубліковано', 'success')
        await loadNews(true)
      } catch (error) {
        const message = error instanceof Error ? error.message : 'Помилка зміни статусу новини'
  ...
  const handleDelete = useCallback(
    ...
    [addToast, items.length, limit, loadNews, offset],
  )

  const requestDelete = useCallback((item: NewsItem) => {
  ...
  return (
    <div className="min-h-screen">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-white mb-2">Новини</h1>
          <p className="text-gray-400 text-sm">Керування стрічкою новин сайту</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="p-12 rounded-xl border-2 border-white/10 bg-black/30 text-center">
          <p className="text-gray-400 text-lg">Новини не знайдено</p>
      ...
      <ConfirmDialog
        ...
        description={itemToDelete ? `Ця дія незворотна. Новина "${itemToDelete.title}" буде видалена остаточно.` : ''}
