...
import {
  createWorkshopItem,
  deleteWorkshopItem,
  fetchWorkshopItemById,
  uploadWorkshopItemInlineImage,
  updateWorkshopItem,
  uploadWorkshopItemCoverImage,
} from '@/actions/workshop-items'
...
import { WORKSHOP_ITEM_CATEGORY_OPTIONS, WorkshopItemFormData } from './types'
...
const EMPTY_FORM: WorkshopItemFormData = {
  title: '',
  slug: '',
  excerpt: '',
  content: '',
...
export default function WorkshopItemEditorPageClient({ mode, workshopItemId }: Props) {
  const router = useRouter()
  const [form, setForm] = useState<WorkshopItemFormData>(EMPTY_FORM)
  ...
  useEffect(() => {
    if (mode !== 'edit' || !workshopItemId || Number.isNaN(workshopItemId)) {
      setIsLoading(false)
      return
    }
    ...
    const loadNews = async () => {
      try {
        setIsLoading(true)
        setLoadError(null)
        const item = await fetchWorkshopItemById(workshopItemId)

        if (cancelled) return

        if (!item) {
          addToast('Послугу не знайдено. Можливо, її вже видалено.', 'warning')
          router.replace('/workshop-items')
          return
        }
        ...
        if (cancelled) return
        setLoadError(error instanceof Error ? error.message : 'Помилка завантаження послуги')
      } finally {
  ...
  }, [addToast, mode, workshopItemId, router])
  ...
  const formError = useMemo(() => {
    ...
    if (!form.category) return 'Оберіть категорію послуги'
  ...
  const handleDelete = useCallback(() => {
    if (mode !== 'edit' || !workshopItemId || Number.isNaN(workshopItemId)) return
  ...
  const handleConfirmDelete = useCallback(async () => {
    ...
    try {
      setIsDeleting(true)
      await deleteWorkshopItem(Number(workshopItemId))
  ...
  const handleSave = async (targetPublished: boolean) => {
    ...
    try {
      ...
      const payload: WorkshopItemFormData = {
        ...form,
      ...
      if (coverFile) {
        const uploadedCoverUrl = await uploadWorkshopItemCoverImage(coverFile)
        payload.coverImage = uploadedCoverUrl
      }

      const saved =
        mode === 'create'
          ? await createWorkshopItem(payload)
          : await updateWorkshopItem(Number(workshopItemId), payload)
      ...
      const successMessage = targetPublished
        ? wasPublished
          ? 'Публікацію оновлено'
          : 'Послугу опубліковано'
        : wasPublished
          ? 'Послугу знято з публікації'
          : 'Чернетку збережено'

      addToast(successMessage, 'success')

      if (mode === 'create') {
        router.replace(`/workshop-items/${saved.id}`)
      } else {
        router.refresh()
      }
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Помилка збереження послуги'
      addToast(message, 'error')
    } finally {
      setIsSaving(false)
    }
  ...
  if (loadError) {
    return (
      <div className="space-y-4">
        <Link href="/workshop-items" className="inline-flex items-center gap-2 text-sm text-gray-300 hover:text-white transition-colors">
          <MdArrowBack size={18} />
          До списку послуг
        </Link>
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white">
            {mode === 'create' ? 'Нова послуга' : 'Редагування послуги'}
          </h1>
          <p className="text-sm text-gray-400 mt-1">Створення та редагування картки майстерні</p>
        </div>

        <div className="flex items-center gap-2">
          <Link href="/workshop-items" className="h-9 px-4 flex items-center rounded-lg border border-white/10 text-gray-200 text-sm hover:bg-white/5 transition-colors">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1.9fr_1fr] gap-6 items-start">
        <div className="space-y-6">
          <section className="p-5 rounded-xl border-2 border-white/10 bg-black/30">
            ...
            />
            <p className="text-xs text-gray-500 mt-1">URL: /workshop/{form.slug || 'slug'}</p>
            ...
            <select
              value={form.category}
              onChange={(e) => setForm((prev) => ({ ...prev, category: e.target.value as WorkshopItemFormData['category'] }))}
              className="w-full px-4 py-2.5 bg-white/5 text-white rounded-lg border border-white/10 focus:border-(--color-primary) focus:outline-none"
          ...
          <section className="p-5 rounded-xl border-2 border-white/10 bg-black/30">
            ...
            <TinyMceEditor
              ...
              onImageUpload={uploadWorkshopItemInlineImage}
              disabled={isSaving || isDeleting}
            />
          </section>
        </div>
      ...
      <ConfirmDialog
        ...
        description={`Ця дія незворотна. Картка${form.title.trim() ? ` "${form.title.trim()}"` : ''} буде видалена остаточно.`}
