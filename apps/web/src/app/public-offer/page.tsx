...
interface PublicOfferTemplateConfig {
  success: boolean;
  data: {
    title: string;
    content: string;
...
export async function generateMetadata(): Promise<Metadata> {
  return buildTemplateMetadata({
    ...
    fallbackTitle: 'Договір публічної оферти | Strike Shop Action',
    fallbackCanonicalPath: '/public-offer',
  });
}
...
export default async function PublicOfferPage() {
  ...
  const data = await getTemplate('public-offer');
  if (!data.success) {
    return (
      <div className="text-center mt-4 text-[grey] ">
        Не вдалося завантажити дані публічної оферти.
      </div>
    );
  }

  const { title, content, seo } = (data as PublicOfferTemplateConfig).data;
